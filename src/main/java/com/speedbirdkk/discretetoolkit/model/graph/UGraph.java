...
import com.speedbirdkk.discretetoolkit.model.relational.Relation;
import com.speedbirdkk.discretetoolkit.utils.Sets;
...
public final class UGraph<A> implements Graph<A> {
    ...
    /**
     * Constructs a new simple undirected graph from a symmetric and irreflexive relation
     * on one set.
     * 
     * <p>From a {@code Relation}, where {@code domain.equals(codomain)} returns true, sets
     * the {@code vertices} of the graph to {@code domain} and the edges of the graph to
     * each of the pairs in {@code relationSet} made unordered. Since the graph is undirected,
     * the relation is required to be symmetric and since the graph has no self-loops,
     * the relation is required to be irreflexive.
     * 
     * @see model.relational.Relation
     * @param <A> the vertex type
     * @param relation the relation from which to construct the graph
     * @throws NullPointerException if relation is null
     * @throws IllegalArgumentException if {@code domain} is not equal to the {@code codomain}
     * @throws IllegalArgumentException if {@code relation} is not symmetric
     * @throws IllegalArgumentException if {@code relation} is not irreflexive
     * @return the graph with vertices as {@code domain} of the relation and edges as the pairs
     * in {@code relationSet} made unordered
     */
    public static <A> UGraph<A> fromRelation(Relation<A, A> relation) {
        Objects.requireNonNull(relation, "Relation must not be null");

        if (!relation.domain().equals(relation.codomain())) {
            throw new IllegalArgumentException("Domain and codomain must be equal");
        }

        if (!Validate.symmetry(relation)) {
            throw new IllegalArgumentException("Relation must be symmetric for undirected graph");
        }

        if (!Validate.irreflexivity(relation)) {
            throw new IllegalArgumentException("Relation must be irreflexive for simple graph");
        }

        Set<UEdge<A>> edges = relation.relationSet().stream()
            .map(pair -> new UEdge<>(pair.a(), pair.b()))
            .collect(Collectors.toSet());
        
        return new UGraph<>(relation.domain(), edges);
    }
